...
class APP {
  ...
  initRouter() {
    ...
    const authenticate = (req, res, next) => {
      ...
      if (
        !credentials ||
        ((credentials.name !== ConfigManager.APP.APP_USER ||
          credentials.pass !== ConfigManager.APP.APP_PASSWORD) &&
          (credentials.name !== ConfigManager.APP.APP_OTHER_USER ||
            credentials.pass !== ConfigManager.APP.APP_OTHER_PASSWORD))
      ) {
    ...
    this.app.get("/*", async (req, res, next) => {
      try {
        ...
        const treeWithUrls = Object.entries(tree).map(([name, value]) => {
          ...
          const urlPath = isFile
            ? ConfigManager.APP.APP_URL +
              "/public/" +
              req.requestedFolder +
              "/" +
              encodeURIComponent(name)
            : path.join("/", req.requestedFolder, encodeURIComponent(name));
        ...
        res.render("index", {
          tree: treeWithUrls,
          previousUrl: ConfigManager.APP.APP_URL + previousUrl,
          isReturn,
          currentFolder: navigationFolder,
        });
      } catch (error) {
        console.error(
          "Erreur lors de la génération de l'arborescence JSON :",
          error,
        );
        res
          .status(500)
          .send(
            "Une erreur est survenue lors de la récupération de l'arborescence.",
          );
      }
    });
  }
